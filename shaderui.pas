...
begin
 ...
 {$ELSE}
   ...
   {$ELSE} //Linux
   kT := GLForm1.Label2.top; //do this dynamically - if user adjusts text size in Windows7, the position of static controls changes!
   {$ENDIF}
 {$ENDIF}
 {$ELSE}
...
{$IFDEF FPC}
 ...
 {$ELSE}
   ...
   {$ELSE}
     result := controlTop(gShader.nUniform+1)+(kH );
    {$ENDIF}
 {$ENDIF}
{$ELSE}
