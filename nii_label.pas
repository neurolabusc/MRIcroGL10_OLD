...
procedure LoadLabelCustomLut(lFileName: string);
implementation
...
var
 ...
 i: integer;
   ...
   lFData: file;
begin
  gLabelLut := defaultLut();
  fnm := changefileext(lFilename, '.lut');
  if not fileexists(fnm) then
     fnm := ChangeFileExtX(lFilename, '.lut');
  if not fileexists(fnm) then exit;
  if FSize(fnm) <> 768 then exit;
  assignfile(lFdata,fnm);
  Filemode := 0;
  reset(lFdata,1);
  GetMem( lBuff, 768);
  ...
  closefile(lFdata);
  for i := 0 to 255 do
      gLabelLut[i] := makeRGB(lBuff[i],lBuff[i+256],lBuff[i+512]);
  gLabelLut[0].rgbReserved:= 0;
  freemem(lBuff);
end;
...
procedure desaturateRGBA( var lRGBA: TGLRGBQuad; frac: single);
var
  r,g,b: byte;
  y: single;
begin
