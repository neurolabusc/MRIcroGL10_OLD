...
interface
 uses
  Classes, SysUtils, nifti_types, define_types;

 function readDICOMHeader (var fname: string; var nhdr: TNIFTIhdr; var gzBytes: int64; var swapEndian: boolean): boolean;
...
end;  //clear_dicom_data

function NIFTIhdr_LoadDCM (var lFilename: string; var lDICOMdata: DICOMdata): boolean;
const
...
begin //function fast_read_dicom_data
     ...
     while (lDICOMData.imagestart = 0) and (not lBufferError) do begin
         ...
         //   msg(VR+inttohex(lGroupElement,8)+'  '+inttostr(lLength));
...
begin
  gzBytes := 0;
  result := NIFTIhdr_LoadDCM (fname, lDICOMdata);
  if not result then exit;
  case lDicomData.Allocbits_per_pixel of
       8: nhdr.datatype := kDT_UNSIGNED_CHAR;
       16: nhdr.datatype := kDT_SIGNED_SHORT;
       32: begin
           if lDicomdata.Float then
              nhdr.datatype := kDT_SIGNED_INT
           else
              nhdr.datatype :=  kDT_FLOAT;     // float (32 bits/voxel)
           end;
       else begin
  ...
  nhdr.vox_offset := lDicomData.ImageStart;
  nhdr.bitpix := lDicomData.Allocbits_per_pixel;
  nhdr.pixdim[1] := lDicomdata.XYZmm[1];
  nhdr.pixdim[2] := lDicomdata.XYZmm[2];
  nhdr.pixdim[3] := lDicomdata.XYZmm[3];
  nhdr.dim[1] := lDicomdata.XYZdim[1];
  nhdr.dim[2] := lDicomdata.XYZdim[2];
  nhdr.dim[3] := lDicomdata.XYZdim[3];
  nhdr.dim[4] := lDicomdata.XYZdim[4];
  if nhdr.dim[4] < 2 then
     nhdr.dim[0] := 3
  else
      nhdr.dim[0] := 4;
  nhdr.qform_code := kNIFTI_XFORM_UNKNOWN;
  nhdr.sform_code := kNIFTI_XFORM_UNKNOWN;
  ...
  nhdr.srow_x[3] := (nhdr.dim[1] /2)*-nhdr.pixdim[1];
  nhdr.srow_y[3] := (nhdr.dim[2] /2)*-nhdr.pixdim[2];
  nhdr.srow_z[3] := (nhdr.dim[3] /2)*-nhdr.pixdim[3];
  nhdr.toffset := 0;
  nhdr.intent_code := kNIFTI_INTENT_NONE;
  nhdr.dim_info := kNIFTI_SLICE_SEQ_UNKNOWN + (kNIFTI_SLICE_SEQ_UNKNOWN shl 2) + (kNIFTI_SLICE_SEQ_UNKNOWN shl 4); //Freq, Phase and Slie all unknown
  nhdr.xyzt_units := kNIFTI_UNITS_UNKNOWN;
  nhdr.slice_duration := 0; //avoid +inf/-inf, NaN
  nhdr.intent_p1 := 0;  //avoid +inf/-inf, NaN
  nhdr.intent_p2 := 0;  //avoid +inf/-inf, NaN
  nhdr.intent_p3 := 0;  //avoid +inf/-inf, NaN
  nhdr.pixdim[0] := 1; //QFactor should be 1 or -1
  swapEndian := not odd(lDicomData.little_endian);
  nhdr.magic := kNIFTI_MAGIC_DCM;
end;
