        ...
        case tTag.tagID of
          ...
          $0103 : begin
               ...
               if (tTag.offset <> 1) and (tTag.offset <> 5) then begin
                  eStr := 'Unsupported compression format '+inttostr(tTag.offset);
                  goto 666;
               end;
...
function Save2Nii(fnm: string; nhdr: TNIFTIhdr; img: byteP0): boolean;
var
  f: file;
  outFnm: string;
  nImgBytes: integer;
...
function SaveTIFFAsNifti(fnm: string): boolean;
var
  s: string;
  nhdr: TNIFTIhdr;
  img: byteP0;
...
var
  ...
  lHdr: TMRIcroHdr;
  lInF: File;
  lVolOffset, lFileBytes, lImgBytes, lVol, i, vox: integer;
  lBuf, lImgBuffer: byteP;
  l32Buf : LongIntP;
  l16Buf : WordP;
begin
  result := true;
  if SaveTIFFAsNifti(fnm) then exit;
  result := false;
  if not NIFTIhdr_LoadHdr(fnm, lHdr, false) then begin
     showmessage('Load error '+fnm);
     exit;
  end;
  for lVol := 4 to 7 do
      lHdr.NIFTIhdr.dim[lVol] := 1;
  lHdr.NIFTIhdr.dim[0] := 3;
  lVol := 1;
  lVolOffset := (lVol-1) * lImgBytes;
  lImgName := lHdr.ImgFileName;
  if not fileexists(lImgName) then begin
     Showmessage('LoadImg Error: Unable to find '+lImgName);
     exit;
  end;
  ...
  if (lHdr.gzBytes = K_gzBytes_headerAndImageUncompressed) and (FSize (lImgName) < (lHdr.NiftiHdr.vox_offset+ lImgBytes)) then begin
       Showmessage(format('LoadImg Error: File smaller (%d) than expected (%d+%d): %s',[FSize (lImgName), round(lHdr.NiftiHdr.vox_offset), lImgBytes,  lImgName]) );
       exit;
  end;
  ...
  if not lHdr.DiskDataNativeEndian then begin
     vox := lHdr.NIFTIhdr.dim[1]*lHdr.NIFTIhdr.dim[2]*lHdr.NIFTIhdr.dim[3];
     l32Buf := LongIntP(lImgBuffer );
     l16Buf := WordP(lImgBuffer );
     if lHdr.NIFTIhdr.bitpix = 32 then
        for i := 1 to vox do
            pswap4i(l32Buf[i]);
     if lHdr.NIFTIhdr.bitpix = 16 then
        for i := 1 to vox do
            l16Buf[i] := swap(l16Buf[i]);
  end;
  result := save2nii(fnm, lHdr.NIFtiHdr, byteP0(lImgBuffer)); ;
end;
