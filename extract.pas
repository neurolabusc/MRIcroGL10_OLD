...
var
   ...
   lMin16 : Smallint;
   lMin32 : single;
   ValueI,lnVox,lVox: integer;
   lMin,lMax,lIntercept,lSlope: single;
begin
  lnVox := lTexture.FiltDim[1]*lTexture.FiltDim[2]*lTexture.FiltDim[3]-1;
   if lnVox < 0 then
      exit;
  lMin := Scaled2Unscaled(gCLUTrec.min,lTexture);
  lMax := Scaled2Unscaled(gCLUTrec.max,lTexture);
  if lMax = lMin then
    exit;
  ...
  if lTexture.RawUnscaledImg16 <> nil then begin //16
    ...
    for lVox := 0 to 255 do begin
            ...
            if ValueI > 255 then
              ValueI := 255;
            lH[lVox] :=  ValueI;//Luminance
    end; //for each vox
    for lVox := 0 to lnVox do
            lM8^[lVox+1] := lH[lTexture.RawUnscaledImg8^[lVox]];
  end; //if 8 bit
  MaskBackground  (lM8, lTexture.FiltDim[1],lTexture.FiltDim[2],lTexture.FiltDim[3],lOtsuLevels, lDilateVox, lOneContiguousObject);
  if lTexture.RawUnscaledImg16 <> nil then begin //16
    lMin16 :=  lTexture.RawUnscaledImg16^[0];
    for lVox := 0 to lnVox do
        if lTexture.RawUnscaledImg16^[lVox] < lMin16 then lMin16 :=  lTexture.RawUnscaledImg16^[lVox];
    for lVox := 0 to lnVox do
        if lM8^[lVox+1] = 0 then
           lTexture.RawUnscaledImg16^[lVox] := lMin16
  end else if lTexture.RawUnscaledImg32 <> nil then begin //32-bit
    lMin32 :=  lTexture.RawUnscaledImg32^[0];
    for lVox := 0 to lnVox do
        if lTexture.RawUnscaledImg32^[lVox] < lMin32 then lMin32 :=  lTexture.RawUnscaledImg32^[lVox];
    for lVox := 0 to lnVox do
        if lM8^[lVox+1] = 0 then
            lTexture.RawUnscaledImg32^[lVox] := lMin32;
  end else if (lTexture.RawUnscaledImg8 <> nil) then begin //8-bit
    lMin8 :=  lTexture.RawUnscaledImg8^[0];
    for lVox := 0 to lnVox do
        if lTexture.RawUnscaledImg8^[lVox] < lMin8 then lMin8 :=  lTexture.RawUnscaledImg8^[lVox];
    for lVox := 0 to lnVox do
        if lM8^[lVox+1] = 0 then
           lTexture.RawUnscaledImg8^[lVox] := lMin8;
  end;
  freemem(lM8);
end;
...
var
  lC,lX,lY,lZ, lXY,lXYZ,lPos,lOffset: integer;
  lTemp: bytep;
begin
   if (lXi < 5) or (lYi < 5) or (lZi < 1) then exit;
   lXY := lXi*lYi; //offset one slice
