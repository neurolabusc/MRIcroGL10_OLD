...
var
  gRayCast: TRayCast;
  procedure glUniform1ix(prog: GLHandleARB; name: AnsiString; value: integer);
  procedure glUniform1fx(prog: GLHandleARB; name: AnsiString; value: single );
  ...
  if (gRayCast.glslprogramSobel = 0) then
    ...
    if (lTex.updateBackgroundGradientsGLSL) then begin
       ...
       if gPrefs.Debug then
          GLForm1.Caption := 'GLSL gradient time '+inttostr((gettickcount-startTime));
    end;
